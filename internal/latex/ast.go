...
// implement one of the following interfaces, or Literal.

// FlexContainer is the interface for containers with a variable number of
// children, supporting append and indexed mutation. The renderer uses
// AppendChildren to build trees as it walks the parser's output.
...
type (
	...
	// EmptyExpr marks the termination of a previous expression. The parser
	// returns one of these when it encounters a stray RBRACE that matches an
	// open brace in the expectation stack, so the closing brace's sibling
	// expressions don't get re-parsed as new top-level statements.
	EmptyExpr struct {
		From, To Pos
		Type     Token
	}
...
func (x *TextStringWrapper) AppendChildren(children ...Expr) { x.Runes = append(x.Runes, children...) }
...
func (x *TextContainer) SetArg(index int, expr Expr) {
	...
	// The parser always wraps \text-like command bodies in a TextStringWrapper
	// before this SetArg is reached. If we get anything else it means the
	// caller is mixing the Node APIs in an unsupported way; recover by
	// wrapping the stray expr so the tree stays walkable instead of crashing
	// the renderer.
	if n, ok := expr.(*TextStringWrapper); ok {
		x.Text = n
	} else {
		x.Text = &TextStringWrapper{Runes: []Expr{expr}}
	}
}
