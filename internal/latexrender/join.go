...
func JoinHorizontal(baseline []int, strs ...string) string {
	...
	if len(baseline) != len(strs) {
		// This is a programmer error: the caller is supposed to pass one
		// baseline per input block. Returning a visible marker instead of
		// panicking keeps a malformed render from crashing the whole
		// renderer pipeline.
		return "[JoinHorizontal: baseline/str count mismatch]"
	}
